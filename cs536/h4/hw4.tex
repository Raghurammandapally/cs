...
<decl> ::= <BOOL> <ID> <SEMICOLON>
    \alt <INT> <ID> <SEMICOLON>

stmt ::= <ID> <ASSIGN> <exp> <SEMICOLON> 
     \alt <IF> <LPAREN> <exp> <RPAREN> <stmt>
     \alt <LCURLY> <list> <RCURLY> 
...
Then, the following translation rules apply:
\texttt{program.trans = list.trans}
\texttt{list$_1$.trans = list$_2$.trans $\cup$ oneItem.trans}
\texttt{list.trans = \{ \}}
\texttt{oneItem.trans = \{ \}}
\texttt{oneItem.trans = stmt.trans}
\texttt{decl.trans = \{ \}}
\texttt{decl.trans = \{ \}}
\texttt{stmt.trans = \{ ID.value \} $\cup$ exp.trans}
\texttt{stmt$_1$.trans = exp.trans $\cup$ stmt$_2$.trans}
\texttt{stmt.trans = list.trans}
\texttt{exp$_1$.trans = exp$_2$.trans $\cup$ exp$_3$.trans}
\texttt{exp$_1$.trans = exp$_2$.trans $\cup$ exp$_3$.trans}
\texttt{exp$_1$.trans = exp$_2$.trans $\cup$ exp$_3$.trans}
\texttt{exp.trans = \{ ID.value \}}
\texttt{exp.trans = \{ \}}
\texttt{exp.trans = \{ \}}
...
%ANSWER
%*********************
Consider the following parse tree, where at each node the translation has been done for all of the nodes below it, according to the translation rules given in problem 1.
\begin{center}
\begin{forest}
